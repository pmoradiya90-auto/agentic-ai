...
from agenticwork.steps.NexusIqReport.NexusIqReport import NexusIqReport
from agenticwork.steps.CloneRepo.CloneRepo import CloneRepo
...
class AutoUpgrade(Step):
    ...
    def run(self) -> dict:
        CloneRepo(self.inputs).run()
        outputs = NexusIqReport(self.inputs).run()
