...
class CreatePR(Step):
    ...
    def __init__(self, inputs: dict):
        ...
        if self.enabled:
            ...
            if "github_api_key" in inputs.keys():
                ...
                self.scm_client = Bitbucket(url=self.bitbucket_url, username=self.bitbucket_user, password=self.bitbucket_access_token)
            else:
                logger.warning(
    ...
    def run(self) -> dict:
        ...
        pull_request = self.scm_client.open_pull_request(source_project=workspace, source_repo=repository_slug, dest_project=workspace, dest_repo=repository_slug, source_branch=source_branch, destination_branch=destination_branch, title=self.title, description=self.pr_body)
