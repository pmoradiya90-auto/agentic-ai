...
class CreatePR(Step):
    ...
    def __init__(self, inputs: dict):
        ...
        self.bitbucket_url = inputs["bitbucket_url"]
        self.bitbucket_access_token = inputs["bitbucket_access_token"]
        self.bitbucket_user = inputs["bitbucket_user"]
        ...
        if self.enabled:
            ...
            elif "bitbucket_access_token" in inputs.keys():
                self.scm_client = Bitbucket(url=inputs["bitbucket_url"], username=self.bitbucket_user, password=inputs["bitbucket_user"])
            else:
                logger.warning(
    ...
    def run(self) -> dict:
        ...
        #repo_slug = get_slug_from_remote_url(original_remote_url).split("/")

        # bitbucket_url = self.bitbucket_url
        # access_token = self.bitbucket_access_token
        # workspace = repo_slug[1]
        # repository_slug = repo_slug[2]
        #
        #
        ...
        #create bitbucket instance with the access token
       # bitbucket = Bitbucket(url=bitbucket_url, username=self.bitbucket_user,password=access_token)
        #pull_request = bitbucket.open_pull_request(source_project=workspace, source_repo=repository_slug, dest_project=workspace, dest_repo=repository_slug, source_branch=source_branch, destination_branch=destination_branch, title=self.title, description=self.pr_body)
        ...
        logger.info(f"Creating PR from {self.base_branch} to {self.target_branch}")
        original_remote_url = repo.remotes[self.original_remote_name].url
        repo_slug = self.get_slug_from_remote_url(original_remote_url)
    ...
    def _create_bitbucket_pr(self, repo_slug: str, title: str, body: str, base_branch: str, target_branch: str) -> str:
        ...
        auth = HTTPBasicAuth(self.bitbucket_user, self.bitbucket_access_token)  # Use your Bitbucket username and App Password
